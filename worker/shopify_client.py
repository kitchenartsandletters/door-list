...
Synchronous Shopify Admin GraphQL client for door-list/worker.

Ported from the sr-ops-suite pattern (Jan 2026 Dev Dashboard migration).
Same TokenManager + client-credentials-grant flow; same env var contract, so
this service can share the SHOPIFY_CLIENT_ID / SHOPIFY_CLIENT_SECRET already
configured on other Kitchen Arts services.
...
class TokenManager:
    ...
    def _refresh_locked(self) -> None:
        ...
        logging.info("[shopify] Access token refreshed (prefix=%s..., expires_in=%ss)", token[:8], expires_in)
...
class ShopifyClient:
    def __init__(self) -> None:
        ...
        self.base_url = f"https://{self._tokens.domain}/admin/api/{api_version}/graphql.json"
    ...
    def validate_connection(self) -> None:
        try:
            ...
            logging.info("[shopify] API connection validated: %s", self.base_url)
        except RuntimeError as e:
            if "Not Found" in str(e) or "404" in str(e):
                raise RuntimeError(
                    "Shopify API version may be sunset. Check SHOPIFY_API_VERSION. "
                    f"URL: {self.base_url}"
                )
            raise
    ...
    def _graphql_once(self, query: str, variables: Optional[Dict[str, Any]], allow_retry: bool) -> Dict[str, Any]:
        ...
        if resp.status_code == 401 and allow_retry:
            logging.warning("[shopify] 401 on GraphQL call; refreshing token and retrying once.")
            self._tokens.invalidate()
            self.session.headers["X-Shopify-Access-Token"] = self._tokens.get_token(force_refresh=True)
